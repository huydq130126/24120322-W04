...
{   // Tao Node moi tu data
    ...
    if (L->pHead ==NULL){ // Neu danh sach lien ket rong 
        ...
        L->pHead ->pPrev = p; // cap nhat con tro pPrev
...
{    // Tao Node moi tu data
    ...
    {
        newNode->pPrev = L->pTail; // Cap nhat con tro pPrev
...
{   // Neu dslk rong thi tra ve false
    ...
    if (L->pHead ==NULL){
        ...
        L->pHead ->pPrev = NULL; // cap nhat lai pPrev cho phan tu dau moi
    }
...
{   
    ...
    dNODE * p = L->pTail;// Cho con tro p chay tu cuoi dslk
    L->pTail = L->pTail ->pPrev; // Lui lai 1 node
    L->pTail ->pNext =NULL; // Ngat node cuoi cu
    delete p; // Xoa node p
}
...
void removeAll(dList *&L)
{
    ...
    while(L->pHead !=NULL){ // Chay cho den khi DSLK rong
        ...
        // Neu dslk chua rong thi ta ngat lien ket tu node dau moi ve node da bi xoa
        if (L->pHead!=NULL){
            L->pHead->pPrev = NULL;
        }
        delete p; // xoa node p
...
{  // Neu dslk rong thi khong lam gi, thoat khoi ham
   ...
   while (p != NULL){
      if (p->key ==val){ // neu node co gia tri bang val
        dNODE *q = p->pPrev; // Tao q la nut dung truoc p
        if (q != NULL){ // preq !=NULL, tuc la node can xoa khong phai node dau
            q ->pNext = p; // Bo qua node q
            delete q; // Xoa node q
        } 
        if (q== L->pHead){
            L->pHead = p;
            p->pPrev = NULL; // Ngat lien ket nguoc tu p ve node cu
        }
        else {  // preq == NULL, tuc la node can xoa la node dau
            q->pPrev ->pNext =p ;// bo qua node q theo chieu xuoi
            p ->pPrev = q ->pPrev; // bo qua node q theo chieu nguoc
        }
        return; // sau khi xoa thi khong lam gi nua, thoat khoi ham
      }
      // Duyet node tiep theo 
      p = p ->pNext;
   }
    ...
    while(p!=NULL){ // Neu p khac NULL, tuc la dslk khac rong
        if (p->key == val){ // neu node co gia tri bang val
            ...
            if (q!=NULL){ // Neu node q khac NULL, tuc la sau sau node p van co phan tu
                ...
                // Neu con node sau q thi cap nhat lien ket nguoc
                if (q->pNext!=NULL){
                    q->pNext->pPrev = p;
                }
                delete q; // xoa node q
...
{   // Neu vi tri la so am thi tra ve false
    ...
    if (p->pNext == NULL){ // Neu p la node cuoi thi dat lai p_tail
        L->pTail = p;
    } else { // Neu p khong la node cuoi 
        q->pNext ->pPrev =p; // cap nhat q tro ve p
...
{   // Neu vi tri la so am thi tra ve false
    ...
    if (q != NULL && q->pNext!=NULL){ // Neu q khac NULL va node sau q khong phai null, tuc la pos nam trong dslk
        dNODE * tmp = q->pNext; // Tao con tro tam de luu node tiep theo cua q
        q->pNext = tmp ->pNext; // bo qua con tro tmp
        
        if (tmp->pNext != NULL) {
            tmp->pNext->pPrev = q; // Cap nhat con tro pPrev sau tmp (neu co)
        }
...
{   // Tao node p tu du lieu data
    ...
    if(L->pHead->key == val){
        ...
        if (L->pHead->pNext!=NULL){ // Neu node sau node dau khong phai NULL
            L->pHead->pNext->pPrev = p; // cap nhat preV cua node sau
        }
       L->pHead ->pNext =p; // Cap nhat pNext cua node dau
       p->pPrev =L->pHead; // Cap nhat pPrev cua node p
        return true; // tra ve true
    }
    // Tao con tro q chay tu dau 
    dNODE *q = L->pHead;
    ...
    if (q!=NULL){
        ...
        p->pNext = q->pNext;
        if(q->pNext != NULL){// Neu q khong phai node cuoi
            q->pNext->pPrev = p; // cap nhat con tro pPrev cua node tiep theo tro ve p 
        }
        
        q->pNext =p; // cap nhat pNext q->p
        p->pPrev = q; // cap nhat pPrev p->q
        if (L->pTail == q){// Neu q la node cuoi thi cap nhat lai node cuoi la p
            L->pTail =p;
            p->pNext = NULL; // Dam bao p la node cuoi
        }
        return true;
    }
...
{   // Neu dslk rong thi ta can tra ve mot dslk moi cung rong
    ...
    while (q != NULL) {
        ...
        if (reverseList->pHead != NULL){
            reverseList ->pHead ->pPrev = newNode; // cap nhat con tro pPrev cua node dau dslk
        }
        ...
        {   // Neu phan tu ngay sau p co gia tri trung voi node q 
            ...
            {  // Tao mot node moi luu dia chi cua node trung
                ...
                if (duplicate == L->pTail) 
                    L->pTail = p;
                // neu pNext khong phai NULL thi ta cap nhat con tro pPrev cho pNext
                if (p->pNext != NULL)
                    p->pNext->pPrev = p;
                // Xoa node trung 
                delete duplicate;
...
{   // Neu ham rong thi tra ve false, khong xoa
    ...
    if (p != NULL) {
        if (q == NULL) { // Node can xoa o dau
            ...
            if(L->pHead!=NULL){ // Neu dslk khong rong thi can cap nhat lai pPrev cua node dau
                L->pHead->pPrev =NULL;
            }
        } else {
            q->pNext = p->pNext; // Noi node truoc voi node sau
            if (p->pNext!=NULL){ // Neu khong phai node cuoi, cap nhat lai pPrev cua node sau
                p->pNext->pPrev = q;
            }
